...
void		op_sti(t_game *game, t_process *process)
{
	...
	&& ft_check_args(game, process->seek, args, 11))
	{
		...
		*(int *)(game->arena + (ft_index_mod(process->seek
		- size[0] - size[1] - 2 + total_index)
		...
		spread_color((ft_index_mod(process->seek
		- size[0] - size[1] - 2 + total_index) % MEM_SIZE), game, process);
		process->index = process->seek;
	}
	else
